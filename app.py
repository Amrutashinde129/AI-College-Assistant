...
# =========================================================
# BEAUTIFUL MODERN UI (UPDATED CSS)
# =========================================================
st.markdown(
    """
<style>
...
.dash-welcome { font-size: 2rem; font-weight: 800; margin-bottom: 8px; }
.dash-date { opacity: 0.9; font-size: 1rem; }
...
.stat-value { color: #1e293b; font-size: 1.75rem; font-weight: 800; margin: 5px 0; }
.stat-sub { color: #94a3b8; font-size: 0.8rem; }
...
.focus-title { font-weight: 800; font-size: 1.2rem; margin-bottom: 5px; }
.focus-desc { opacity: 0.8; font-size: 0.95rem; }
...
.action-icon { font-size: 1.5rem; margin-bottom: 8px; display: block; }
.action-label { font-weight: 600; color: #334155; font-size: 0.9rem; }
...
.task-subject { font-size: 0.8rem; color: #64748b; margin-top: 2px; }
...
.subject-name { font-weight: 800; color: #1e293b; font-size: 1.1rem; }
.subject-full { color: #64748b; font-size: 0.85rem; margin-bottom: 12px; }
.progress-bar-bg {
    background: #f1f5f9;
    height: 8px;
    border-radius: 4px;
...
.progress-text { font-size: 0.8rem; color: #64748b; text-align: right; }

/* General Card Style for other pages */
.card {
    background: white;
    border: 1px solid #e2e8f0;
    border-radius: 16px;
...
if st.session_state.page == "dashboard":
    ...
    # Get tasks from session state or create defaults
    tasks = st.session_state.get("generated_tasks", [])
    ...
    completed_count = len(st.session_state.get("completed_tasks", set()))
    task_progress = round((completed_count / total_tasks) * 100) if total_tasks else 0

    # 1. Hero Header
    st.markdown(
        f"""
        <div class="dash-header">
            <div class="dash-welcome">Hello, {safe_html(display_name)}! 👋</div>
            <div class="dash-date">
    ...
    stat_data = [
        {"icon": "🔥", "label": "Study Streak", "value": "7 days", "sub": "Keep momentum", "color": "#f59e0b"},
        {"icon": "⏳", "label": "Exam Countdown", "value": f"{days_left} days", "sub": "Stay consistent", "color": "#3b82f6"},
        {"icon": "✅", "label": "Tasks Done", "value": f"{completed_count}/{total_tasks}", "sub": f"{task_progress}% complete", "color": "#10b981"},
        {"icon": "📈", "label": "Overall Progress", "value": f"{task_progress}%", "sub": "Learning journey", "color": "#8b5cf6"},
    ]

    for col, data in zip(stats_cols, stat_data):
        with col:
            st.markdown(
                f"""
                <div class="stat-card" style="border-top: 4px solid {data['color']};">
    ...
    with main_left:
        # Today's Focus
        st.markdown('<div class="section-header">🎯 Today\'s Priority</div>', unsafe_allow_html=True)
        st.markdown(
            """
            <div class="focus-card">
                <div class="focus-title">📚 Deep Study Session</div>
                <div class="focus-desc">Recommended: 45 minutes • One topic • Zero distractions</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        actions = [
            {"icon": "🤖", "label": "Smart Chat", "page": "smart_chat"},
            {"icon": "📝", "label": "MCQ Gen", "page": "mcq_generator"},
            {"icon": "📄", "label": "Summarize", "page": "summarizer"},
            {"icon": "❓", "label": "Imp. Qs", "page": "important_questions"},
        ]
        
        for col, act in zip(action_cols, actions):
            with col:
                st.markdown(
                    f"""
                    <div class="action-btn" onclick="document.getElementById('btn_{act['page']}').click()">
                ...
                if st.button("", key=f"btn_{act['page']}", help=f"Go to {act['label']}"):
                    st.session_state.page = act["page"]
                    st.rerun()
        ...
        if tasks:
            for i, task in enumerate(tasks):
                ...
                # Display task card manually to control layout better than standard checkbox
                status_color = "#10b981" if is_completed else "#cbd5e1"
                title_style = "text-decoration: line-through; color: #94a3b8;" if is_completed else ""
                
                st.markdown(
                    f"""
                    <div class="task-item" style="border-left: 4px solid {status_color};">
                        <div class="task-content">
                            <div class="task-title" style="{title_style}">{safe_html(task.get('title', 'Task'))}</div>
                            <div class="task-subject">{safe_html(task.get('subject', ''))}</div>
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
            ...
            st.info("No tasks generated yet. Visit the Study Planner!")

    with main_right:
        # Subject Progress
        st.markdown('<div class="section-header">📚 Subject Progress</div>', unsafe_allow_html=True)
        
        subjects = [
            {"short": "DBMS", "full": "Database Mgmt", "prog": 78},
            {"short": "OOP", "full": "Object Oriented Prog", "prog": 68},
            {"short": "DSU", "full": "Data Structures", "prog": 61},
            {"short": "DTE", "full": "Digital Techniques", "prog": 52},
        ]
        ...
        for sub in subjects:
            st.markdown(
                ...
                <div class="subject-card">
                    <div class="subject-name">{sub['short']}</div>
                    <div class="subject-full">{sub['full']}</div>
                    <div class="progress-bar-bg">
                        <div class="progress-bar-fill" style="width: {sub['prog']}%"></div>
                    </div>
                    <div class="progress-text">{sub['prog']}% Completed</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            ...
            <div class="info-card">
                <b>🤖 AI Coach Tip:</b><br>
                Based on your recent activity, try focusing on <b>Data Structures</b> today. 
                Use the <b>MCQ Generator</b> to test your knowledge after studying.
            </div>
            """,
            unsafe_allow_html=True,
        )
